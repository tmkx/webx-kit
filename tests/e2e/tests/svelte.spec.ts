...
test('Options Page', async ({ getURL, page }) => {
  ...
  await updateFile('src/pages/options/app.svelte', (content) => content.replace('Options Page', randomID));
...
test('Content Scripts', async ({ page }) => {
  ...
  await updateFile('src/content-scripts/hello.svelte', (content) => content.replace('Edit to Show HMR', randomID));
